...
namespace FastPcapng
{
    ...
    /// <summary>
    /// Lazy .pcapng reader which keeps the data resting on disk as long as possible
    /// </summary>
    public class WeakPcapngFileHandle
    {
        private List<long> _cachedOffsets = null;
        private List<InterfaceDescriptionBlock> _cachedIfacesBlock;

        private FastPcapngBinReader _fastBinReader;
        public string Path { get; set; }


        public WeakPcapngFileHandle(string path)
        {
            Path = path;
            _fastBinReader = new FastPcapngBinReader();
        }
        ...
        public List<long> GetPacketsOffsets()
        {
            if (_cachedOffsets == null)
            {
                ...
                _cachedOffsets = _fastBinReader.ReadEpbOffsets(fs);
            }
        ...
        public List<InterfaceDescriptionBlock> GetInterfaces()
        {
            if (_cachedIfacesBlock == null)
            {
                ...
                // Reading Section header to skip it
                var (firstBlockType, firstBlockLen) = _fastBinReader.ReadBlockTypeLen(binReader);
                ...
                // Interfaces
                _cachedIfacesBlock = _fastBinReader.ReadInterfaceDescBlocks(binReader);
            }
